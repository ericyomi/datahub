...
@Slf4j
public class DataProductUsageStatsResolver implements DataFetcher<CompletableFuture<UsageQueryResult>> {

  private final UsageClient usageClient;

  public DataProductUsageStatsResolver(final UsageClient usageClient) {
  ...
  public CompletableFuture<UsageQueryResult> get(DataFetchingEnvironment environment) throws Exception {
    ...
    return CompletableFuture.supplyAsync(() -> {
      if (!isAuthorized(resourceUrn, context)) {
        log.debug("User {} is not authorized to view usage information for data products {}",
            context.getActorUrn(),
  ...
  private boolean isAuthorized(final Urn resourceUrn, final QueryContext context) {
    return AuthorizationUtils.isAuthorized(context,
        ...
        PoliciesConfig.VIEW_DATA_PRODUCT_USAGE_PRIVILEGE);
  }
}
